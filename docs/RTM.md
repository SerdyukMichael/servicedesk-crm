...
| R1-7 | BR-F-102 | История курсов иностранных валют к системной валюте; ручная установка (admin/accountant); набор валют не фиксирован | Must | UC-1003 | `models/exchange_rate.py`, `endpoints/settings.py` | T-1-007 | 🔄 |
...
| R1-10 | BR-F-126 | Редактирование акта при существующем счёте разрешено только `admin`; остальные роли получают 403 `ACT_LOCKED_INVOICE_EXISTS` | Must | — | `endpoints/tickets.py` (`update_work_act` guard) | `tests/test_work_act_invoice_lock.py` | ✅ |
| R1-11 | BR-F-127 | Автосинхронизация счёта при редактировании акта: если счёт не оплачен — заменяются позиции и пересчитываются итоги; если счёт оплачен — 409 `INVOICE_PAID_MISMATCH`; `force_save=true` сохраняет акт без изменения счёта | Must | — | `endpoints/tickets.py` (`_sync_invoice_from_act`, `_calc_act_total`) | `tests/test_work_act_invoice_lock.py` | ✅ |
